(() => {
  const LIMIT = 25;
  const EXPECTED_PHRASE = 'Olá, gostaria de informações sobre agendamento com a Dra. Janifer.';
  const OTHER_PHRASE = 'Olá, gostaria de saber mais sobre o acompanhamento com a Dra. Janifer';
  ...
  if (!plannedIds.length) {
    console.warn('Nenhum resultado encontrado. Pesquise informações sobre agendamento com a Dra. Janifer antes de rodar.');
    return [];
  }
  ...
  (async () => {
    console.log(`Rodada: agendamento. Frase esperada: ${EXPECTED_PHRASE}`);
    ...
    const summary = results.reduce((acc, item) => {
      ...
      return acc;
    }, { rodada: 'agendamento', validos: 0, descartados: 0, por_origem: {} });

    window.__kommoAgendamentoResultados = results;
    window.__kommoAgendamentoResumo = summary;
    console.log('Resumo da rodada:', summary);
    console.log('Copiar JSON: copy(JSON.stringify(window.__kommoAgendamentoResultados, null, 2))');
    console.log('Copiar resumo: copy(JSON.stringify(window.__kommoAgendamentoResumo, null, 2))');
  })();

  return 'Scanner agendamento v5 iniciado.';
})();
